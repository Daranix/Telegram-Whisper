...
getLatestRevision() {
	...
	if [ "$(git tag -l *latest* | wc -l)" -eq "0" ]; then
                local LATEST_COMMIT="$(git rev-list --full-history HEAD | head -n 1)";

		outLog ":latest doesn't exist. Setting :latest to initial commit.";
		outLog "Initial Commit: $LATEST_COMMIT";

        git tag latest $LATEST_COMMIT;
        git tag v1.0.0 $LATEST_COMMIT;
